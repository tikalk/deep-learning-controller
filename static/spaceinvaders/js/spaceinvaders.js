...
Game.prototype.StartGame = function () {
    if (this.currentState() && this.currentState().StartGame) {
        ...
        game.start();
        game.mute();
        document.querySelector('#train').style.opacity = '1.0';
        document.querySelector('#train').disabled = false;
    }
};

Game.prototype.moveToTheRight = function () {
    let keyCode = 39;
    this.pressedKeys[keyCode] = true;
    if (this.currentState() && this.currentState().keyDown) {
        this.currentState().keyDown(this, keyCode);
...
Game.prototype.moveToTheLeft = function () {
    let keyCode = 37;
    this.pressedKeys[keyCode] = true;
    if (this.currentState() && this.currentState().keyDown) {
        this.currentState().keyDown(this, keyCode);
...
WelcomeState.prototype.enter = function (game) {
    ...
    game.sounds.init();
    game.sounds.loadSound('shoot', '/static/spaceinvaders/sounds/shoot.wav');
    game.sounds.loadSound('bang', '/static/spaceinvaders/sounds/bang.wav');
    game.sounds.loadSound('explosion', '/static/spaceinvaders/sounds/explosion.wav');
};
...
WelcomeState.prototype.draw = function (game, dt, ctx) {
    ...
    ctx.fillText("Tensor Alien Invasion", game.width / 2, game.height / 2 - 40);
    ctx.font = "16px Arial";

    ctx.fillText("Train your model and let's play!", game.width / 2, game.height / 2);
};
...
GameOverState.prototype.draw = function (game, dt, ctx) {
    ...
    ctx.font = "16px Arial";
    ctx.fillText("Train your model and let's play!", game.width / 2, game.height / 2 + 40);
    document.querySelector('#train').style.opacity = '1.0';
    document.querySelector('#train').disabled = false;
...
GameOverState.prototype.StartGame = function (game) {
    //  Starts the game.
    game.level = 1;
    game.score = 0;
    game.lives = 3;
...
PlayState.prototype.update = function (game, dt) {
    ...
    // the condition statement is set to true for automatics firing
    if (game.pressedKeys[32] || true) {
        this.fireRocket();
    }
